...
func NewSources(ctx context.Context, cfg *config.Config) (downstream Source, upstream Source, err error) {
	...
	upstream, err = buildSourceFromCfg(ctx, tableDiffs, cfg, cfg.Task.SourceInstances...)
	if err != nil {
		return nil, nil, errors.Annotate(err, "from upstream")
	}
	downstream, err = buildSourceFromCfg(ctx, tableDiffs, cfg, cfg.Task.TargetInstance)
	if err != nil {
...
func buildSourceFromCfg(ctx context.Context, tableDiffs []*common.TableDiff, cfg *config.Config, dbs ...*config.DataSource) (Source, error) {
	if len(dbs) < 1 {
	...
	if ok {
		if len(dbs) == 1 {
			return NewTiDBSource(ctx, tableDiffs, dbs[0], cfg.CheckThreadCount, cfg.UseStaleRead)
		} else {
			log.Fatal("Don't support check table in multiple tidb instance, please specify one tidb instance.")
		}
	}
	return NewMySQLSources(ctx, tableDiffs, dbs, cfg.CheckThreadCount)
}
